...
public class MineSweeper {
    ...
    public void tryExpose(int row, int col){
        ...
        else if (square.hasMine()) { drawLose(); }
    ...
    public void exposeSquareAt(int row, int col){
        ...
        if (square.getAdjacentMines() == 0) {
            square.setExposed();
            square.draw(TOP+col*SQUARE_SIZE, LEFT+row*SQUARE_SIZE, SQUARE_SIZE);
    ...
    public void mark(int row, int col){
        ...
        else if (square.isMarked()){ square.toggleMark(); }
        else                       { square.toggleMark(); }
        square.draw(TOP+col*SQUARE_SIZE, LEFT+row*SQUARE_SIZE, SQUARE_SIZE);
    }
    ...
    public boolean hasWon(){
        for (int row = 0; row < ROWS; row++) {
            for (int col = 0; col < COLS; col++) {
                Square square = squares[row][col];
                if (square.hasMine() && square.isExposed()) { return true; }
            }
        }
        return false;
    }
