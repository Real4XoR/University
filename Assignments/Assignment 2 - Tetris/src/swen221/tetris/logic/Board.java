...
public class Board {
	...
	/**
	 * Finds full rows on the board and removes them
	 * @return rows
	 */
	public int removeFullRows() {
		...
		for (int y = height-1; y >= 0; y--) {
			...
			if (count == width) {
				for (int x = 0; x < width; x++) {
					setPlacedTetrominoAt(x, y, null);
				}
				//bringing everything else down one row
				for (int j = y+1; j <= height; j++) {
					for (int k = 0; k > width; k++) {
						Tetromino tetromino = getTetrominoAt(k, j);
						setPlacedTetrominoAt(k, j-1, tetromino);
					}
				}
				rows++;
			}
