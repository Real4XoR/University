...
public class WaveformAnalyser{
    ...
    public void display(){
        ...
        for (int Y = 0; Y < this.waveform.size()-1; Y++) {
            X1_Counter +=1;
            X2_Counter += 1;
            UI.setColor(Color.blue);
            if (Y == 0) {
                UI.drawLine(X1_Counter, ZERO_LINE, X2_Counter, ZERO_LINE + waveform.get(Y));
            }
            else {
                UI.drawLine(X1_Counter, ZERO_LINE + waveform.get(Y), X2_Counter, ZERO_LINE + waveform.get(Y+1));
                UI.println(waveform.get(Y));
            }
        }
    ...
    public void showSpread() {
        ...
        double max_value = Collections.max(waveform);
        double min_value = Collections.min(waveform);
        double spread = min_value - max_value;
        
        this.display();
        
        UI.println("Max value is: " + max_value);
        UI.println("Min value is: " + min_value);
        UI.println("Total spread is: " + spread);
        // drawing max and min value line aswell as printing the min max on the graph        
        UI.setColor(Color.green); 
        UI.drawString("Max Value is: " + max_value, GRAPH_LEFT + 50, ZERO_LINE - max_value - 10);
        UI.drawLine(GRAPH_LEFT, ZERO_LINE - max_value, GRAPH_RIGHT, ZERO_LINE - max_value);
        
        UI.setColor(Color.red);
        UI.drawString("Min value is: " + min_value, GRAPH_LEFT + 50, ZERO_LINE - max_value + 10);
        UI.drawLine(GRAPH_LEFT, ZERO_LINE + min_value, GRAPH_RIGHT, ZERO_LINE + min_value);
    }
    ...
    public void doubleFrequency() {
        ...
        for (int i = 0;i < waveform.size()-1; i += 2) {
            waveform.remove(i);
        }
