...
public class OrganisationChart {
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        for (Position subPos : pos.getTeam()) {
            Position findChild = findPosition(x, y, subPos);
            if (findChild != null) {
                findPos = findChild;
            }
        }
    ...
    private void movePosition(Position pos, Position target) {
        ...
        if (!inSubtree(pos, target)) {
            movePos = pos;
            movePos.getManager().removeFromTeam(pos);
            movePos.addToTeam(target);
        }
    }
    ...
    public void removePosition(Position pos){
        if (!pos.isManager()) {
            Position oldManager = pos.getManager();
            oldManager.removeFromTeam(pos);
            UI.println("Inside the removePosition method. Pos = " + pos);
        } else if (pos.isManager()) {
            UI.println("Cannot remove position with subordinates");
        }
    }
