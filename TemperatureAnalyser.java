...
public class TemperatureAnalyser{
    ...
    public void printAverage(ArrayList<Double> listOfNumbers) {
        ...
        for (Double value : listOfNumbers) { 
            average += value.doubleValue(); //for each loop to grab the average number
        }
    
        average /= listOfNumbers.size(); // dividing average by the amount of numbers in list
        
        UI.printf("Todays average was %2f\n", average);
    }
    ...
    public void plotLevels(ArrayList<Double> listOfNumbers) {
        ...
        for (int i = 0; i < listOfNumbers.size(); i++) {
            if (listOfNumbers.get(i) >= 400) {
                UI.drawString("*", left+(step*i)+5, 20); //Completion task to draw asterisk
                UI.fillRect(left+(step*i), base - (listOfNumbers.get(i)), 20,(listOfNumbers.get(i)));
            }
            else {
                UI.fillRect(left+(step*i), base - (listOfNumbers.get(i)), 20,(listOfNumbers.get(i)));
            }
            UI.fillRect(left, base, (listOfNumbers.get(i)), 1); // drawing x axis in the bottom
        }
    ...
    public double maximumOfList(ArrayList<Double> listOfNumbers) {
        ...
        double max_level = listOfNumbers.get(0); 
        
        for (int i = 1; i < n; i++) {
            if (listOfNumbers.get(i) > max_level) {
                max_level = listOfNumbers.get(i);
            }
        }
        return max_level;
    }
    ...
    public double minimumOfList(ArrayList<Double> listOfNumbers) {
        int n = listOfNumbers.size();
        double min_level = listOfNumbers.get(0);
        
        for (int i = 1; i < n; i++) {
            if (listOfNumbers.get(i) < min_level) {
                min_level = listOfNumbers.get(i);
            }
        }
        return min_level;
    }
